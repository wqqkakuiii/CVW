...
func TestGetWrappedInstance(t *testing.T) {
	...
	wasmBytes, contractId, logger := prepareContract("./testdata/rust-func-verify-2.0.0.wasm", t)
	...
	for i := 0; i < 22; i++ {
		...
		go func(no int) {
			...
			fmt.Printf("vmPool used %v instane.\n", atomic.LoadInt32(&vmPool.useCount))
			time.Sleep(waitTime)
		}(i)
	}
	wg.Wait()
...
func TestGrowAndShrink(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func(no int) {
			...
			fmt.Printf("vmPool used %v instane, currentSize = %v \n", atomic.LoadInt32(&vmPool.useCount), atomic.LoadInt32(&vmPool.currentSize))
			time.Sleep(waitTime)
		}(i)
	}
	wg.Wait()
	...
	assert.Equal(t, vmPool.currentSize, int32(5), "new vmPool should has 10 instances.")
	...
	for i, wrappedInstance := range instancePool {
		fmt.Printf("revert %v instance, currentSize = %v \n", i, atomic.LoadInt32(&vmPool.currentSize))
		vmPool.RevertInstance(wrappedInstance)
	}
	assert.Equal(t, vmPool.currentSize, int32(35), "new vmPool should has 20 instances.")

	vmPool.shrink(defaultDelayTolerance)
	assert.Equal(t, vmPool.currentSize, int32(25), "new vmPool should has 10 instances after shrink.")

...
//		CaptureStdout().
//	  Finalize()
func (self *WasiStateBuilder) Finalize(store *C.wasm_store_t) (*WasiEnvironment, error) {
	return newWasiEnvironment(self, store)
}
...
func newWasiEnvironment(stateBuilder *WasiStateBuilder, store *C.wasm_store_t) (*WasiEnvironment, error) {
	var environment *C.wasi_env_t

	err := maybeNewErrorFromWasmer(func() bool {
		environment = C.wasi_env_new(store, stateBuilder.inner())
...
func (self *WasiEnvironment) Initialize(store *Store, instance *Instance) error {
	...
	err := maybeNewErrorFromWasmer(func() bool {
		...
		return false == C.wasi_env_initialize_instance(self.inner(), store.inner(), instance.inner())
	})
	if err != nil {
		return err
	}

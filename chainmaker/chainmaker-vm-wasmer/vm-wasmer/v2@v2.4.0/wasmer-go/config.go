...
func IsCompilerAvailable(compiler CompilerKind) bool {
	return bool(C.wasmer_is_backend_available(uint32(C.wasmer_backend_t(compiler))))
}
...
func (self *Config) UseCraneliftCompiler() *Config {
	...
	C.wasm_config_set_backend(self.inner(), uint32(C.wasmer_backend_t(CRANELIFT)))
...
// MaxPagesLimit限制实例内存上限，默认256页，每页64KB，即16MB
// 如果设置上限太小，小于异常会报Failed to create memory: A user-defined error occurred: Minimum exceeds the allowed memory limit的错误
// 下限无法设置，由编译器和wasmer决定
func (self *Config) MaxPagesLimit(maxPagesLimited uint32) *Config {
	C.wasm_config_set_max_pages(self.inner(), C.uint32_t(maxPagesLimited))
	return self
}

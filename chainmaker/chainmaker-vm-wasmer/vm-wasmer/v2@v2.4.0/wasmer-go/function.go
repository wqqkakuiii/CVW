...
func (self *Function) Type() *FunctionType {
	...
	self.funcType = newFunctionType(ty, self.ownedBy())
...
func (self *Function) Native() NativeFunction {
	...
	expectedReturnNum := len(ty.Results())
	...
	self.lazyNative = func(receivedParameters ...interface{}) (interface{}, error) {
		...
		C.wasm_val_vec_new_uninitialized(&results, C.size_t(expectedReturnNum))
...
func (self *hostFunctions) load(index uint32) (*hostFunction, error) {
	self.RLock()
...
func (self *hostFunctions) store(hostFunction *hostFunction) uint32 {
	self.Lock()
	defer self.Unlock()
	...
	self.functions[self.currPos] = hostFunction
	return self.currPos
}

func (self *hostFunctions) remove(index uint32) {
	self.Lock()
	defer self.Unlock()

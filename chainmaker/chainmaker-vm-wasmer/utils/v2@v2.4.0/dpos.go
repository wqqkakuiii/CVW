...
func verifyDPosConfig(cc *configPb.ChainConfig, ercCfg *ERC20Config, stakeCfg *StakeConfig) error {
	...
	for _, candidate := range stakeCfg.candidates {
		if NewBigInteger(candidate.Weight).Cmp(NewBigInteger(stakeCfg.minSelfDelegation)) < 0 {
			return fmt.Errorf("candidate %s is below minimum self-delegation requirement (required: %s, actual: %s)",
				candidate.PeerId,
				stakeCfg.minSelfDelegation,
				candidate.Weight,
			)
		}
	}

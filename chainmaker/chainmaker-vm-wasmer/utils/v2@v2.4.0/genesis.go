...
const (
	// default timestamp is "2020-11-30T01:01:01+08:00"
	defaultTimestamp           = int64(1606669261)
	errMsgMarshalChainConfFail = "proto marshal chain config failed, %s"
	genesisNoteKey             = "GENESIS_NOTES"
	// GenesisNoteTag 是用于标识创世区块的标签
	GenesisNoteTag = "genesis_note"
)

// CreateGenesis create genesis block (with read-write set) based on chain config
func CreateGenesis(cc *configPb.ChainConfig, genesisNotes string) (*commonPb.Block, []*commonPb.TxRWSet, error) {
	var (
		err      error
	...
	if len(cc.GenesisTime) > 0 {

		if setTimestamp, err = rfc3339ToUninTime(cc.GenesisTime); err != nil {
			return nil, nil, fmt.Errorf("time conversion failed (chainconfig.genesisTime), %s", err)
		}
	}

	// generate config tx, read-write set, and hash
	if tx, err = genConfigTx(cc, setTimestamp, genesisNotes); err != nil {
		return nil, nil, fmt.Errorf("create genesis config tx failed, %s", err)
	}

	if rwSet, err = genConfigTxRWSet(cc, setTimestamp); err != nil {
...
// RFC 3339 规定了一种非常严格的日期和时间格式，包括日期、时间和时区等信息。
// 其中最常用的格式是 "YYYY-MM-DDTHH:mm:ssZ"，其中 T 用来分隔日期和时间， Z 用来标识时区。
// 入参需要设置为 "2024-10-30T00:00:00+08:00" 类似这种格式。可参照测试方法func TestGetUnixTime(t *testing.T)。
func rfc3339ToUninTime(strTime string) (int64, error) {
	// 将时间字符串转换为time.Time类型
	parsedTime, parseErr := time.Parse(time.RFC3339, strTime)
	if parseErr != nil {
		return 0, fmt.Errorf("time conversion failed (chainconfig.genesisTime), %s", parseErr)
	}
...
func genConfigTx(cc *configPb.ChainConfig, timestamp int64, genesisNotes string) (*commonPb.Transaction, error) {
	var (
		err     error
		ccBytes []byte
		//payloadBytes []byte
	...
	payload := &commonPb.Payload{
		...
		TxId:         GetTxIdWithSeed(timestamp),
		Timestamp:    timestamp,
	}
	...
	if strings.TrimSpace(genesisNotes) != "" {
		payload.Parameters = append(payload.Parameters, &commonPb.KeyValuePair{
			Key:   genesisNoteKey,
			Value: []byte(genesisNotes),
		})
	}
...
func genConfigTxRWSet(cc *configPb.ChainConfig, timestamp int64) (*commonPb.TxRWSet, error) {
	var (
		err         error
		ccBytes     []byte
...
func totalTxRWSet(cc *configPb.ChainConfig, chainConfigBytes []byte,
	...
	for _, k := range syscontractKeys {
		...
		//在2312的时候引入的TRANSACTION_MANAGER，在之前版本中不需要初始化
		if name == syscontract.SystemContract_TRANSACTION_MANAGER.String() &&
			ccVersion < GetBlockVersion("v2.3.1.2") {
			continue
		}
		//240引入SYNC_RULE合约, 在之前版本中不需要初始化
		if name == syscontract.SystemContract_SYNC_RULE.String() &&
			ccVersion < protocol.BlockVersion240 {
			continue
		}
